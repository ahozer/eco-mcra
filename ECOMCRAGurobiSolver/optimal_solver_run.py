...
import json 
import sys, getopt
def gurobi_solve(bids,dimension,phy_resource_capacities,phymac_idlefullenergies,data_centers_physical_machines,index):
    pue=1.57
    allocation_time=720
 ...
 #   sum_of_total_energy_cost=0
   ...
   #     sum_of_total_energy_cost=sum_of_total_energy_cost+idle_power+utilization_power
    ...
    if status != 3 and status !=4 : 
        ...
        bid_acceptance_ratio= accepted*100/len(bids)
        
        optimal_total_price=model.getVarByName("z0").X
        optimal_total_idle_energy=model.getVarByName("z1").X
        optimal_total_util_energy=model.getVarByName("z2").X
        number_of_machines_powered_on=model.getVarByName("z3").X



        result_dictionary["obj-value"]=optimal_value
        result_dictionary["gurobi-runtime"]=model.Runtime
        result_dictionary["solution-stat"] =status
        result_dictionary["MIP-gap"]=mip_gap
        result_dictionary["bid-acceptance-ratio"]=bid_acceptance_ratio
        result_dictionary["optimal-total-price"]=optimal_total_price
        result_dictionary["optimal-total-idle-energy"]=optimal_total_idle_energy
        result_dictionary["optimal-total-util-energy"]=optimal_total_util_energy   
        result_dictionary["num-of-machines-powered-on"]=number_of_machines_powered_on 
        
        model.write("solution_"+str(index)+".sol")

        model.write("model_"+str(index)+".lp")

    else:
        result_dictionary["obj-value"]=0
        result_dictionary["gurobi-runtime"]=model.Runtime
        result_dictionary["solution-stat"] =status
        result_dictionary["MIP-gap"]=0
        result_dictionary["bid-acceptance-ratio"]=0
        result_dictionary["optimal-total-price"]=0
        result_dictionary["optimal-total-idle-energy"]=0
        result_dictionary["optimal-total-util-energy"]=0   
        result_dictionary["num-of-machines-powered-on"]=0     
...
results=gurobi_solve(bids,total_dimension,phymacs,phymac_idlefullenergy,phy_datacenter,test_case_no)
...
"gurobi-runtime":results["gurobi-runtime"],
"status":results["solution-stat"],
"MIP-Gap":results["MIP-gap"],
"bid-acceptance-ratio":results["bid-acceptance-ratio"],
"optimal-total-price":results["optimal-total-price"],
"optimal-total-idle-energy":results["optimal-total-idle-energy"],
"optimal-total-util-energy":results["optimal-total-util-energy"],
"num-of-machines-powered-on":results["num-of-machines-powered-on"]                    
}

gurobi_results.append(dic)

gurobi_dic={"gurobi-results":gurobi_results}
with open("gurobi_"+str(test_case_no)+".json", "w") as outfile:
    json.dump(gurobi_dic, outfile)